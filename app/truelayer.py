...
authorization_base_url = "https://auth.truelayer.com/?response_type=code&client_id="+client_id+"&nonce=1586027006&scope="+scopes+"&redirect_uri="+redirect_uri+"&enable_mock=true"
...
def get_accounts():
    ...
    for results in resp_dict['results']:
		...
		if 'account_id' in results:
			html=html+results['account_id']
		html=html+"</td>"
        html=html+"<td>"
		if 'account_type' in results:
			html=html+results['account_type']
		html=html+"</td>"
        html=html+"<td>"
		if 'display_name' in results:
			html=html+results['display_name']
		html=html+"</td>"
        html=html+"<td>"
		if 'currency' in results:
			html=html+results['currency']
		html=html+"</td>"
        html=html+"<td>"
		if 'iban' in results['account_number']:
			html=html+results['account_number']['iban']
		html=html+"</td>"
        html=html+"<td>"
		if 'swift_bic' in results['account_number']:
			html=html+results['account_number']['swift_bic']
		html=html+"</td>"
        html=html+"<td>"
		if 'number' in results['account_number']:
			html=html+results['account_number']['number']
		html=html+"</td>"
        html=html+"<td>"
		if 'sort_code' in results['account_number']:
			html=html+results['account_number']['sort_code']
		html=html+"</td>"
        html=html+"</tr>"
